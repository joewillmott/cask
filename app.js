...
(function () {
    ...
    function writePreviewStyles() {
        ...
        const decl = (prop, val) => val && val !== '' ? `    ${prop}: ${val};\n` : '';
...
${hideTitle}${hideBreadcrumb}${hideTags}`.trim();
    ...
    async function exportCSS() {
        ...
        // Build the new @import block from whatever the user has in the font imports field
        let imports = '';
        if (v['font-imports']) {
            v['font-imports'].split('\n').forEach(line => {
                const u = line.trim();
        ...
        // Build the new :root block from the current admin field values
        const rootBlock =
`:root {
    /* Colours */
    --color-bg:          ${v['color-bg']};
    ...
    --radius-md:         ${v['radius-md']};
        ...
        // If the fetch fails (static host, network error), fall back to a variables-only export
        // with a clear warning comment so the user knows what they have.
        let css;
        try {
            ...
            // Replace the existing @import block (everything before the first non-@import,
            // non-comment, non-whitespace line) with the new one, then replace the :root block.
            let updated = original;

            // 1. Replace @import lines at the top of the file with the new imports block.
            //    Match a run of @import statements (with optional surrounding whitespace/comments).
            updated = updated.replace(/^([ \t]*@import\s[^;]+;[ \t]*\n)+/m, imports || '\n');

            // 2. Replace the :root { ... } block (including its contents) with the new one.
            //    The regex is non-greedy and handles nested braces by matching up to the
            //    first } that closes the opening brace at the :root level.
            updated = updated.replace(/:root\s*\{[^}]*\}/s, rootBlock);

            css = updated;
        } catch (_) {
            // Fallback: variables-only export with a warning
            css =
`/* ============================================================
   cask.ink — stylesheet (variables only)
   ============================================================
   WARNING: The full style.css could not be fetched from the
   server. This file contains ONLY the :root variables block.
   Do NOT use it as a drop-in replacement for style.css —
   paste the :root block into your existing style.css instead.
   ============================================================ */
